        ...
        <div className={styles.modalBody}>
          <div className={styles.returnBadge}>
            <span className={feedback.returnAsTeacher ? styles.yes : styles.no}>
              {feedback.returnAsTeacher ? '✓ Would return as teacher' : '✗ Would not return as teacher'}
            </span>
          </div>
...
export default function CourseFeedbacks() {
  ...
  useEffect(() => {
    const fetchAll = async () => {
      ...
      try {
        const [fbRes, stRes] = await Promise.allSettled([
          axios.get(`http://localhost:3000/api/admin/courses/${encoded}/feedbacks`, {
            headers: { Authorization: `Bearer ${token}` }
          }),
          axios.get(`http://localhost:3000/api/admin/courses/${encoded}/statistics`, {
            headers: { Authorization: `Bearer ${token}` }
          })
        ])
        if (fbRes.status === 'fulfilled') setFeedbacks(fbRes.value.data.feedbacks)
        else setError('Failed to load feedbacks')
        if (stRes.status === 'fulfilled') setStats(stRes.value.data.statistics)
      } catch (err) {
        ...
        <div className={`${styles.tableWrap} fade-up fade-up-delay-2`}>
          <table className={styles.table}>
            ...
            <tbody>
              {feedbacks.length === 0 ? (
                <tr>
                  <td colSpan="6" className={styles.emptyRow}>NO FEEDBACKS FOUND</td>
                </tr>
              ) : (
                feedbacks.map((fb, i) => (
                  <tr key={fb._id} className={styles.tr}>
                    ...
                    <td className={styles.td}>
                      <span className={fb.returnAsTeacher ? styles.yesTag : styles.noTag}>
